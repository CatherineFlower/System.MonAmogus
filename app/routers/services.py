...
def index_page(request: Request) -> HTMLResponse:
    return templates.TemplateResponse(request, "index.html", {"user_ip": _get_public_ip()})


@web_router.get("/services/{id}", response_class=HTMLResponse)
def service_detail_page(id: int, request: Request, db: Session = Depends(get_db)) -> HTMLResponse:
    ...
    return templates.TemplateResponse(
        request,
        "service_detail.html",
        {"service": service, "checks": checks, "events": events, "current_status": current_status},
    )
    ...
    for item in services:
        ...
        result.append(
            ServiceOut(
                ...
                last_check_status=(
                    "available" if last_check and last_check.is_available else ("unavailable" if last_check else None)
                ),
